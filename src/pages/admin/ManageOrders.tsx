...
import { Trash2, Edit, ChevronDown, CheckCircle, Package, Link as LinkIcon, Download } from 'lucide-react';
...
export default function ManageOrders() {
  ...
  const saveDownloadUrl = async (id: string) => {
    try {
      await updateDoc(doc(db, 'orders', id), { downloadUrl, status: 'Delivered' });
                ...
                {orders.map(order => (
                  <React.Fragment key={order.id}>
                    <tr className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      <td className="py-4">
                        <div className="text-sm font-mono text-gray-400">{order.id.slice(-6)}</div>
                        <div className="text-xs text-gray-500">{new Date(order.createdAt).toLocaleDateString()}</div>
                      </td>
                      <td className="py-4 text-sm">
                        <div className="font-medium text-white">{order.customerName || order.customerEmail || 'N/A'}</div>
                        <div className="text-gray-500 text-xs">{order.customerPhone}</div>
                      </td>
                      <td className="py-4 text-sm font-medium">{order.templateName || 'Custom'}</td>
                      <td className="py-4 text-sm font-medium text-emerald-400">₹{order.price || order.amount || 0}</td>
                      <td className="py-4 text-sm">
                         <select 
                           value={order.status || 'Pending'} 
                           onChange={(e) => updateStatus(order.id, e.target.value)}
                           className={`bg-gray-800 border-none rounded px-2 py-1 text-xs font-bold font-sans ${order.status==='Delivered' ? 'text-purple-400' : order.status==='Completed' ? 'text-green-400' : 'text-yellow-400'}`}
                         >
                      ...
                      <tr className="bg-gray-800/20">
                         <td colSpan={6} className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                  ...
                                  <div className="space-y-2 text-sm">
                                    ...
                                    <div className="flex text-gray-400 items-center">
                                     ...
                                     <div className="flex text-gray-400 items-center">
                                        ...
                                        <span className="text-orange-400 font-bold">₹{order.advancePayment || 0}</span>
                               ...
                               <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
                                  <h4 className="text-white font-bold mb-3 flex items-center gap-2"><CheckCircle className="w-4 h-4"/> Delivery</h4>
                                  <div className="space-y-4">
                                     {order.downloadUrl ? (
                                        <div className="text-sm bg-green-500/10 border border-green-500/20 text-green-400 p-3 rounded-xl flex items-start gap-2">
                                          ...
                                          <div>
                                            <div className="font-bold mb-1">Link Sent to Client</div>
                                            <a href={order.downloadUrl} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline break-all">{order.downloadUrl}</a>
                                          </div>
                                        </div>
                                     ) : (
                                        <div className="text-sm text-gray-400">
                                          No delivery link attached yet. Upload the final video to Google Drive/YouTube and paste the link below.
                                        </div>
                                     )}
                                     
                                     <div className="flex gap-2">
                                        <input type="url" placeholder="https://drive.google.com/..." value={downloadUrl} onChange={e=>setDownloadUrl(e.target.value)} className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white" />
                                        <button onClick={() => saveDownloadUrl(order.id)} disabled={!downloadUrl} className="bg-indigo-500 hover:bg-indigo-600 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
